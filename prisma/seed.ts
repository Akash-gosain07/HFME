...
async function persistAggregatedMetrics(
  ...
  for (const step of steps) {
    ...
    for (const sessionEvents of sessions.values()) {
      ...
      totalRetries += sessionEvents.filter((event) => event.type === 'validation_error').length;
      totalBackNav += sessionEvents.filter((event) => event.type === 'back_navigation').length;
      ...
      if (!sessionEvents.some((event) => event.type === 'step_completed')) {
        droppedSessions++;
      }
    }
